export const STRIPE_PRICE_IDS: Record<string, string> = {
  'newin-7': 'price_1TlFOu3NFaqaTHGIxSYWnI97',
}

export const IMAGES: (string | string[])[] = [
  ["/SHOP/Pa_01.jpg", "/NEW IN/P_01.jpg", "/NEW IN/zoom in/Pb_01.jpg", "/SHOP/plum.jpg"],
  ["/NEW IN/P_02.jpg", "/NEW IN/zoom in/Pb_02.jpg", "/SHOP/fig.jpg"],
  ["/SHOP/Pa_03.jpg", "/NEW IN/P_03.jpg", "/NEW IN/zoom in/Pb_3.jpg", "/SHOP/orbit.jpg"],
  ["/SHOP/Pa_04.jpg", "/NEW IN/P_04.jpg", "/NEW IN/zoom in/Pb_04.jpg", "/NEW IN/zoom in/Pb_05.jpg"],
  ["/NEW IN/P_05.jpg", "/NEW IN/zoom in/Pb_05.jpg", "/SHOP/nebula2.jpg"],
  ["/SHOP/Pa_06.JPG", "/NEW IN/P_06.jpg", "/NEW IN/zoom in/Pb_06.jpg", "/SHOP/orion.jpg"],
  ["/SHOP/Pa_07.jpg", "/NEW IN/P_07.jpg", "/NEW IN/zoom in/Pb_07.jpg", "/SHOP/eclipse.jpg"],
  ["/SHOP/Pa_08.jpg", "/NEW IN/P_08.jpg", "/NEW IN/zoom in/Pb_08.jpg", "/SHOP/chita.jpg"],
  ["/SHOP/Pa_09.jpg", "/NEW IN/P_09.jpg", "/NEW IN/zoom in/Pb_09.jpg", "/SHOP/pitaya.jpg"],
  ["/NEW IN/P_10.jpg", "/NEW IN/zoom in/Pb_10.jpg", "/SHOP/lemon.jpg"],
  ["/NEW IN/P_11.jpg", "/NEW IN/zoom in/Pb_11.jpg"],
]

export const PRICES = [
  2650,
  2750,
  2350,
  2650,
  2750,
  2650,
  5,
  2750,
  2550,
  2350,
  2350,
  2000,
]

export const DESCRIPTIONS: (string | null)[] = [
  "Oaxaca black clay - Indian ceramic - Tiger eye - Hematit",
  "UNKNOWN clay piece - Oaxaca black clay - African krobo glass - coral - stainless steel - mother of pearl shell",
  "VINTAGE PORCELAINE ROSE BEAD - GREEN AVENTURINE - STAINLESS STEEL - Hematite",
  "MOOKAITE JASPER - GREEN MURANO - STAINLESS STEEL - Hematite",
  "RAW MILY QUARTZ FROM MARFA - BLUE GLASS - STAINLESS STEEL - Hematite",
  "AFRICAN BONE BEAD - ITALIAN recycled GLASS - green german vintage glass from the 70s - STAINLESS STEEL - Hematite",
  "OAXACA BLACK CLAY - AFRICAN KROBO RECYCLED GLASS",
  "UNKNOWN RED Painted seed - ITALIAN recycled GLASS - wood chitah bead - Hematite",
  "DALMATIAN JASPER - CORAL - OAXACA BLACK CLAY - STAINLESS STEEL",
  "Indian Dyed Stone Bead in Neon Green - PALM SEED - HEMATITE - STAINLESS STEEL",
  "PAKISTAN CLAY BEAD - CORAL - JADE - STAINLESS STEEL - Hematite",
  null,
  null,
  null,
  null,
]

// Mark which products are sold. Product 7 (index 6) is sold.
export const SOLD = [
  false,
  false,
  false,
  false,
  false,
  false,
  true,
  false,
  false,
  false,
  false,
]

// Initial stock for each product (1 item each necklace)
export const STOCK = IMAGES.map(() => 1)
